...
const Navbar: React.FC<NavbarProps> = ({ pharmacyName, currentPage, onNavigate, isDarkMode, toggleDarkMode }) => {
  ...
  return (
    <nav className={`fixed w-full top-0 z-[60] transition-all duration-500 ${
      ...
      <div className="max-w-[1900px] mx-auto px-4 sm:px-10 lg:px-16">
        <div className="flex justify-between items-center">
          <div className="flex items-center min-w-0">
            ...
            >
              ...
              <span className="ml-2 sm:ml-4 text-lg md:text-xl font-black tracking-tighter text-slate-900 dark:text-white transition-colors truncate uppercase">
                New-Health <span className="text-emerald-600">Pharmacy</span>
              </span>
            </button>
          </div>
          
          <div className="hidden lg:flex lg:space-x-16 xl:space-x-32 items-center flex-grow justify-center">
            {navLinks.map((link) => (
              <button 
                key={link.id}
                onClick={() => handleNavClick(link.id)} 
                className={`flex items-center font-black transition-all border-b-2 px-1 py-1 space-x-3 text-[10px] uppercase tracking-[0.4em] whitespace-nowrap ${
                  currentPage === link.id 
          ...
          <div className="hidden lg:flex items-center space-x-8">
            ...
            <button 
              ...
              className="bg-emerald-600 text-white px-10 py-3 rounded-full font-black text-[10px] uppercase tracking-[0.3em] hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/20 active:scale-95 flex items-center space-x-2 whitespace-nowrap"
            >
